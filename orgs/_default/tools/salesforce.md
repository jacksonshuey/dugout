...
- **API call budget:** Enterprise baseline is "100,000 + 1,000 per Salesforce license" per 24-hour rolling window (per Platform API limits cheat sheet). A 50-seat customer gives us 150K calls/day shared with every other integration on the org. Our adapter has to operate inside the leftover headroom, which in practice means CDC-first (one subscription, server-pushed) rather than poll-first.
