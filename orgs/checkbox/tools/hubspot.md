...
- **What it is:** Contact at an active-opp account views CMS pages tagged as competitor-comparison (`/vs/ironclad`, `/vs/linksquares`, `/vs/agiloft`, `/alternatives/*`) or downloads a competitor-switching guide.
...
4. **Custom lifecycle stages.** `GET /crm/v3/properties/contacts/lifecyclestage` → enumerate `options[]`. Checkbox may have added stages between SQL and Opportunity (e.g., "Selected Vendor"); the ordinal map in signal 3 needs to include them.
