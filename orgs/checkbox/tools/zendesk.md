...
- **SFDC linking field:** Confirm whether Checkbox already populates `organization.external_id` (preferred — single field, indexed) or needs a new custom organization field `sfdc_account_id` provisioned via Admin Center → Objects and rules → Organization fields. The field key becomes the lookup path: `organization.organization_fields.sfdc_account_id`. If neither exists, install creates the custom field and a one-time backfill script keyed off domain match.
- **CSAT trigger config:** Verify a Trigger exists for "Satisfaction rating updated → score is Bad" posting to the Dugout webhook endpoint. If Checkbox is on Professional+ but hasn't enabled CSAT surveys (`satisfaction_rating.score` will be `"unoffered"` across the board), the champion-CSAT half of Signal 2 degrades to priority-only and we lean harder on `escalation` tags.
