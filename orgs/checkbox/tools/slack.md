...
- **Why for the wedge:** This is the literal demo example from spec §1.4: *"Slack shows the AE asked the SE for a KPI assessment, but there is no completed handoff."* Selected Vendor stalls partly because internal Checkbox people drop the ball on what the buyer asked for. Catching the dropped handoff is the rare case where Slack is genuinely the only source.
...
- **What it is:** A Checkbox employee posts a deal-risk-shaped message in a channel that mentions a known account, without a corresponding Salesforce Close Date push, Stage regression, or Next Step update within 48h. Phrasings: "Acme is wobbly", "I'm worried about renewal at Acme", "Acme keeps rescheduling", "champion at Acme went quiet".
...
- **What it is:** A customer-named or customer-shared Slack channel (e.g., `#acme-checkbox`, a Slack Connect channel with the buyer) has zero buyer-side messages for ≥14 days during an active implementation or expansion cycle. Distinguishes by `user.team_id` ≠ Checkbox's team to detect buyer vs Checkbox activity.
...
- `components/slack-channel-allowlist.tsx` — install-time UI listing all channels with checkbox per channel (default all unchecked)
...
- **User → role → Checkbox-employee resolution.** Match `users.profile.email` domain against the Checkbox domain to tell internal vs buyer. Match individual emails against `workspace_users.email` to resolve role (AE, SE, CS, RevOps, Manager). Unmatched Checkbox-domain users drop into an unassigned bucket for one-click role tagging.
